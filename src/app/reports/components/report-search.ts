...
@Component({
  selector: 'analytics-report-search',
  template: `
    <mat-card>
      <mat-card-title>Find a Report</mat-card-title>
      <mat-card-content>
        <mat-input-container>
          <input matInput placeholder="Search for a report" [value]="query" (keyup)="search.emit($event.target.value)">
        </mat-input-container>
